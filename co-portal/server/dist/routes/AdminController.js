...
router.get("/transaction/all", function (req, res) {
  _Transaction2.default.find({
    $or: [{
      source: "PROPERTY"
    }, {
      source: null
    }]
  }, "_id adminID amount type rentTenantName rentMonth description date").populate("adminID", "userName").then(function (transactions) {
...
router.get("/transaction/:source/all", function (req, res) {
  ...
  _Transaction2.default.find({
    source: source
  }, "_id adminID amount type itemCount carName propertyName productName source rentTenantName rentMonth description date").populate("adminID", "userName").then(function (transactions) {
    if (transactions == null) {
      res.status(500);
      res.send("Error : 9032rtu834g9erbo");
    }
